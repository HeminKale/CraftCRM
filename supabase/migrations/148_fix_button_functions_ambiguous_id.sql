-- Migration: 148_fix_button_functions_ambiguous_id.sql
-- Fix ambiguous column reference 'id' error in button functions
...
-- Function to create a new button - FIXED: Explicit column references
...
BEGIN
  -- Extract tenant_id from button_data (frontend passes it in the JSON)
  _tenant_id := (p_button_data ->> 'tenant_id')::UUID;
  
  -- Debug: Log the values we're working with
  RAISE NOTICE 'Debug: tenant_id = %, object_id = %, button_data = %', _tenant_id, p_object_id, p_button_data;
  
  -- Check if tenant_id is valid
  IF _tenant_id IS NULL THEN
    RAISE EXCEPTION 'Tenant ID not found in button data';
  END IF;
  
  -- Check if object exists and belongs to tenant
  SELECT EXISTS(
    SELECT 1 FROM tenant.objects o
    WHERE o.id = p_object_id AND o.tenant_id = _tenant_id
  ) INTO _object_exists;
  
  IF NOT _object_exists THEN
    -- Provide more detailed error information
    IF NOT EXISTS(SELECT 1 FROM tenant.objects WHERE tenant.objects.id = p_object_id) THEN
      RAISE EXCEPTION 'Object with ID % does not exist', p_object_id;
    ELSE
      RAISE EXCEPTION 'Object with ID % exists but does not belong to tenant %', p_object_id, _tenant_id;
    END IF;
  END IF;
  ...
  ) RETURNING tenant.button__a.id INTO _button_id;

  -- Return the created button with explicit table references
  RETURN QUERY
  SELECT 
    b.id,
    b.name::TEXT,
...
-- Function to update a button - FIXED: Explicit column references
...
DECLARE
  _tenant_id UUID;
BEGIN
  -- Extract tenant_id from button_data
  _tenant_id := (p_button_data ->> 'tenant_id')::UUID;
  ...
  UPDATE tenant.button__a SET
    name = COALESCE(p_button_data ->> 'name', tenant.button__a.name),
    button_type__a = COALESCE(p_button_data ->> 'button_type', tenant.button__a.button_type__a),
    is_active = COALESCE((p_button_data ->> 'is_active')::BOOLEAN, tenant.button__a.is_active),
    label__a = COALESCE(p_button_data ->> 'label', tenant.button__a.label__a),
    custom_component_path__a = COALESCE(p_button_data ->> 'custom_component_path', tenant.button__a.custom_component_path__a),
    custom_route__a = COALESCE(p_button_data ->> 'custom_route', tenant.button__a.custom_route__a),
    action_type__a = COALESCE(p_button_data ->> 'action_type', tenant.button__a.action_type__a),
    action_config__a = COALESCE(p_button_data -> 'action_config', tenant.button__a.action_config__a),
    button_style__a = COALESCE(p_button_data ->> 'button_style', tenant.button__a.button_style__a),
    button_size__a = COALESCE(p_button_data ->> 'button_size', tenant.button__a.button_size__a),
    display_order__a = COALESCE((p_button_data ->> 'display_order')::INTEGER, tenant.button__a.display_order__a),
    updated_at = NOW()
  WHERE tenant.button__a.id = p_button_id;
...
BEGIN
  ...
  WHERE tenant.button__a.id = p_button_id AND tenant.button__a.tenant_id = p_tenant_id;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration 148 completed: Fixed ambiguous column reference in button functions';
END $$;
