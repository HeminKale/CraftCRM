-- Migration 125: Fix schema reference issue in enhanced get_object_records_with_references function
-- Purpose: Fix the "relation tenant.system.users does not exist" error
-- Status: Corrects schema handling for system vs tenant tables

-- Drop and recreate the enhanced function with proper schema handling
...
COMMENT ON FUNCTION tenant.get_object_records_with_references(UUID, INTEGER, INTEGER) IS 'Enhanced: Automatically resolves reference fields to display names using JOINs with proper schema handling';
COMMENT ON FUNCTION public.get_object_records_with_references(UUID, UUID, INTEGER, INTEGER) IS 'Enhanced bridge: Returns resolved reference field names instead of UUIDs with proper schema handling';

-- Log successful migration
DO $$
BEGIN
    RAISE NOTICE '🚀 Migration 125: Fixed schema reference issue in enhanced function!';
    RAISE NOTICE '✅ Fixed: tenant.system.users does not exist error';
    RAISE NOTICE '✅ Enhanced function now properly handles auth.users, system vs tenant schemas';
    RAISE NOTICE '✅ Auth.users table will be referenced correctly (Supabase built-in)';
    RAISE NOTICE '✅ System.users table will be referenced correctly (if exists)';
    RAISE NOTICE '✅ Tenant tables will be referenced correctly';
    RAISE NOTICE '✅ All reference fields now properly resolved with correct schema handling';
END $$;
