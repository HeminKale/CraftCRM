-- ============================================================
-- Migration 267: Recertification — Sprint 3
-- NCR + RCA + Evidences (the Stage-2-shaped round)
--
-- Part of UAF New Changes/New Help Doc/Recertification/00_Sprint_Plan.md,
-- Sprint 3. Structurally closer to External Client Stage 2's evidences
-- round than to Surveillance 1, which has no evidences checkpoint at all —
-- per the plan doc's precedent table, the NCR/RCA half mirrors
-- surv_ncr/surv_ncr_rca/review_surv_ncr_rca (258/259) and the evidences
-- half mirrors stage2_evidences/review_stage2_evidences (234).
--
-- Two deliberate choices, both specified directly in the plan doc (not
-- re-derived here):
--   1. review_recert_ncr_rca AND review_recert_evidences are both gated to
--      the SPECIFIC auditor assigned via assign_recert_team (admin
--      bypasses; no CRM bypass) — mirrors 258's improvement over 234's
--      plain "any Auditor role" check. Since Sprint 2 already built the
--      assignment infrastructure, leaving either accept/reject open to ANY
--      auditor would make assign_recert_team decorative for these two
--      checkpoints.
--   2. recert_ncr_rca upload does NOT clear recert_rca_rejection_notes__a —
--      built in from the start to match the corrected behavior from
--      Surveillance 1's migration 259 (which had to revert an unintended
--      258 deviation). Cleared ONLY inside review_recert_ncr_rca's accept
--      branch, exactly like review_stage1_ncr_rca/review_surv_ncr_rca.
--      recert_evidences upload follows the same convention for symmetry —
--      review_recert_evidences's accept branch is the only place that
--      clears recert_evidences_rejection_notes__a.
--
-- 1. New columns on tenant.recertification_clients__a.
-- 2. Register Sprint 3 fields in tenant.fields.
-- 3. Register this sprint's five new status__a picklist values.
-- 4. RPC: review_recert_ncr_rca — Auditor/admin, specific-assigned-auditor.
-- 5. RPC: review_recert_evidences — Auditor/admin, specific-assigned-auditor.
-- 6. start_file_upload — reproduces 266's full body verbatim + adds
--    recert_ncr (CRM-or-Auditor), recert_ncr_rca (Client-only), and
--    recert_evidences (Client-only) hard gates.
-- 7. finalize_file_upload — reproduces 266's full body verbatim + adds
--    the three upload auto-advance blocks, each with a re-upload status
--    guard from day one (same "don't wait for a follow-up migration"
--    convention Sprint 2 already applied to recert_audit_plan).
-- ============================================================
...
-- recert_evidences__a defaults to '[]'::jsonb (not '{}') — files-type field,
-- matches stage2_evidences__a's shape (234) exactly: an array finalize_file_
-- upload appends to via jsonb concatenation, not a single-object slot.

-- ── 2. Register Sprint 3 fields ─────────────────────────────────────
DO $$
DECLARE
  _tenant_id UUID;
  _object_id UUID;
...
-- ── 4. RPC: review_recert_ncr_rca ────────────────────────────────────
-- Auditor/admin only — deliberately NOT widened to CRM (matches
-- review_surv_ncr_rca / row 12's CRM-view-only rule). Further restricted to
-- the SPECIFIC auditor assigned via assign_recert_team.
-- accept -> Recert_Auditor_Accepted + stamp date + clear rejection notes.
-- reject -> stays Recert_NCR_Sent + store notes, client re-uploads RCA.
...
-- ── 5. RPC: review_recert_evidences ──────────────────────────────────
-- Auditor/admin, specific-assigned-auditor, mirrors review_stage2_evidences
-- (234) with 258's assignment-specificity improvement layered on.
-- accept -> Recert_Evidences_Accepted + stamp date + clear rejection notes.
-- reject -> back to Recert_Auditor_Accepted (client re-uploads evidences
-- only, RCA itself isn't reopened) + store notes.
...
-- ── 6. start_file_upload (REDEFINED — reproduces 266's full body
-- verbatim + adds recert_ncr / recert_ncr_rca / recert_evidences hard
-- gates) ──────────────────────────────────────────────────────────────
...
-- ── 7. finalize_file_upload (REDEFINED — reproduces 266's full body
-- verbatim + adds the recert_ncr / recert_ncr_rca / recert_evidences
-- auto-advance blocks, each with a re-upload status guard from day one) ──
