-- ============================================================
-- Migration 287: Let a Surveillance 1 client be recognized by the
--                 record's own Email field, not only by the linked
--                 External Client's client_user_id__a
--
-- Background: client access to a Surveillance 1 record has always been
-- gated on renewal_clients__a.client_user_id__a, which is only ever
-- populated by copying it from the SELECTED External Client's own
-- client_user_id__a at record-creation time (256). That column, in turn,
-- is only auto-set by a trigger (215) when the External Client record was
-- inserted by a user who themselves holds the External Client role
-- (self-signup) — when CRM/Admin creates the External Client record on
-- the client's behalf (the normal case), it's easy to forget to set it,
-- silently breaking Accept/upload for that client with no visible error.
--
-- Fix: every place that currently checks
--   renewal_clients__a.client_user_id__a = auth.uid()
-- now ALSO accepts a case-insensitive match between the caller's own
-- login email (system.users.email) and the Surveillance 1 record's own
-- email__a field (the one entered on the Surveillance 1 form itself,
-- independent of the External Client record). In practice the two emails
-- are expected to be the same person, so this is purely additive — a
-- superset of who's allowed in, never a narrower one. Scoped to
-- Surveillance 1 (renewal_clients__a) only, per this turn's instruction —
-- Recertification/External Client keep the old client_user_id__a-only
-- check for now.
--
-- Three call sites updated, each reproducing its current live body
-- verbatim with only the client-identity check changed:
--   1. review_surveillance_intimation (live body: 221)
--   2. review_surv_plan               (live body: 257)
--   3. start_file_upload's surv_ncr_rca block (live body: 278 — the
--      full function is reproduced here since it's a single shared RPC)
-- ============================================================

-- ── 1. review_surveillance_intimation ──────────────────────────────
...
BEGIN
  _caller_id := auth.uid();
  SELECT su.tenant_id, su.role, su.email INTO _tenant_id, _caller_role, _caller_email
  FROM system.users su WHERE su.id = _caller_id;

  SELECT r.client_user_id__a, r.email__a INTO _client_user_id, _record_email
  ...
  IF _caller_role != 'admin'
     AND _client_user_id != _caller_id
...
-- ── 2. review_surv_plan ─────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.review_surv_plan(UUID, TEXT, TEXT);
...
-- ── 3. start_file_upload — surv_ncr_rca block only, full body
--       reproduced verbatim from 278 otherwise ─────────────────────
...
BEGIN
  ...
  -- ── surv_ncr_rca: client-only. UPDATED (287) — now also accepts a
  -- case-insensitive match on the record's own email__a, not only
  -- client_user_id__a (see migration header for why). ──
