-- Migration 127: Simplified reference resolution - focus on Channel Partner first
-- Purpose: Get Channel Partner references working without depending on user tables
-- Status: Minimal working solution for reference field resolution

-- Drop and recreate the enhanced function with simplified reference handling
...
-- Create simplified enhanced function that only handles tenant table references
...
COMMENT ON FUNCTION tenant.get_object_records_with_references(UUID, INTEGER, INTEGER) IS 'Simplified: Resolves tenant table reference fields to display names (Channel Partner, etc.)';
COMMENT ON FUNCTION public.get_object_records_with_references(UUID, UUID, INTEGER, INTEGER) IS 'Simplified bridge: Returns resolved reference field names for tenant tables only';

-- Test the enhanced function with a sample object to verify Channel Partner resolution
DO $$
DECLARE
    v_test_object_id UUID;
    v_test_result record;
BEGIN
    -- Get a sample object ID (first object with channelPartner field)
    SELECT f.object_id INTO v_test_object_id
    FROM tenant.fields f
    WHERE f.name = 'channelPartner' 
    AND f.type = 'reference'
    AND f.reference_table IS NOT NULL
    LIMIT 1;
    ...
    IF v_test_object_id IS NOT NULL THEN
        ...
        END LOOP;
    ELSE
        RAISE NOTICE '⚠️ No test object found with channelPartner field';
    END IF;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '🚀 Migration 127: Simplified reference resolution completed!';
    RAISE NOTICE '✅ Enhanced function created with tenant table support only';
    RAISE NOTICE '✅ Channel Partner references will now resolve to company names';
    RAISE NOTICE '✅ All tenant table reference fields properly handled';
    RAISE NOTICE '✅ Skipped system.users/auth.users for now (can add later)';
    RAISE NOTICE '✅ No more schema errors for tenant tables';
END $$;
