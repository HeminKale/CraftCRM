-- Migration 080: Fix get_object_records autonumber handling
-- Purpose: Prevent bigint conversion errors when autonumber columns contain empty strings

-- Drop and recreate the tenant function with safe autonumber handling
...
COMMENT ON FUNCTION tenant.get_object_records(UUID, INTEGER, INTEGER) IS 'Fixed: Safe autonumber handling - skips autonumber column to prevent bigint conversion errors';

-- Log successful migration
DO $$
BEGIN
    RAISE NOTICE '🚀 Migration 080: get_object_records autonumber fix completed!';
    RAISE NOTICE '✅ Autonumber column now safely skipped to prevent bigint errors';
    RAISE NOTICE '✅ Empty string handling improved for bigint columns';
    RAISE NOTICE '✅ No more "invalid input syntax for type bigint: """ errors when reading records';
END $$;
