-- Migration: 147_fix_button_functions_tenant_param.sql
-- Fix button functions to use passed tenant_id parameter instead of JWT
...
-- Function to create a new button - FIXED: Use passed tenant_id parameter
...
BEGIN
  ...
  IF NOT _object_exists THEN
    ...
    IF NOT EXISTS(SELECT 1 FROM tenant.objects WHERE id = p_object_id) THEN
...
-- Function to update a button - FIXED: Use passed tenant_id parameter
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migration 147 completed: Fixed button functions to use passed tenant_id parameter';
END $$;
